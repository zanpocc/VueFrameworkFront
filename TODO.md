...
前端目标：Vue 3 + TypeScript + Vite + Yarn 企业后台平台，支持真实登录、动态菜单、按钮权限、系统管理、异步任务和完整自动化测试。
...
历史记录：工作流和动态表单已从当前前端脚手架移除。本节已完成项仅用于保留历史，不代表当前页面或接口仍然存在。
...
3. 补齐角色关联用户视图。
4. 完善对象存储后端标识和诊断信息增强。
5. 完善异步任务详情、任务参数展示和 Outbox 手动重试。
6. [x] 固定成熟后台视觉基线，建立 token、页面壳、页头、工具栏、卡片和表格容器。
7. [x] 剩余业务页面迁移到共享组件并统一页面观感；新增 `yarn lint:ui` 防止视觉规则回流。
8. [deferred] Playwright 视觉回归：等待确定截图基线后单独接入，本轮不做。
